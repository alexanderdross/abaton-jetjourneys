...
| 2.4 | **Local currency display.** The currency set is confirmed (EUR, USD, CAD, JPY, AUD) and built. Four smaller calls remain open, D2 to D5 in [CURRENCY-CONVERSION-PLAN.md](./CURRENCY-CONVERSION-PLAN.md) section 10: safety margin, automatic detection by visitor country, placement on the cards, and legal sign-off on the new privacy-page cookie section, specifically whether the country-derived `abaton_fx_geo` counts as strictly necessary. | Live since 11 August 2026, drawing real ECB rates. EUR stays the contractual price, an indicative converted line sits next to it, `src/lib/fx/*` and `src/components/fx/*`. The privacy page now has a cookie section naming all three cookies the site sets. | Nothing blocked, all four are built to a documented default. The display should not go live before 2.1 is settled, since converting an unconfirmed price list only multiplies the uncertainty. |
...
| 6.3 | **Go-live switches.** The site is deliberately parked: `NEXT_PUBLIC_NOINDEX="1"`, interim hosting on the account's `*.workers.dev` subdomain (no custom domain), empty Turnstile keys (running on always-pass test keys), and `NEXT_PUBLIC_CF_IMAGES="0"` so raw originals are served instead of edge-optimised images. | `wrangler.jsonc`. | Launch. See `DEPLOYMENT.md` for the cutover checklist. |
